...
const Reminders = () => {
  ...
  // In Reminders.js - handleSaveReminder function
  const handleSaveReminder = async (reminderData) => {
    try {
      let updatedReminders;

      if (reminderToEdit) {
        // Editing existing reminder - keep the same numeric ID
        ...
        // Adding new reminder - ID will be generated by context
        const newReminder = await addReminder(reminderData); // Let context handle ID generation
  ...
  const stats = [
    {
      ...
      value: userReminders.length,
      icon: "bell",
      color: "bg-cyan-500",
    },
    {
      title: "Active",
      value: userReminders.filter((r) => r.isActive).length,
      icon: "check-circle",
      color: "bg-emerald-500",
    },
    {
      title: "Notifications",
      value: userReminders.reduce(
                ...
                <View className="flex-row items-center justify-between">
                  <View className="flex-1">
                    ...
                    <Text className="text-sm text-slate-500 mt-2 font-medium">
                      {userReminders.filter((r) => r.isActive).length} active
                      reminders
                    </Text>
                  </View>
                ...
                <View className="flex-row items-start justify-between">
                  <View className="flex-1 pr-3">
                    ...
                    <View className="flex-row justify-between mb-3">
                      <View>
                        <Text className="text-2xl font-semibold text-slate-800">
                          {userReminders.filter((r) => r.isActive).length}
                        </Text>
